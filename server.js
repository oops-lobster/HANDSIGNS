...
const lexicalFallbacks = new Map([
  ["반드시", ["필수", "꼭"]],
  ["꼭", ["필수"]],
  ["착용", ["입다"]]
]);
const phraseTransparentModifiers = new Set(["많다"]);
...
- "반드시", "꼭", "필수", "-해야 한다"처럼 의무/강조를 나타내는 표현은 국립국어원 검색 가능성이 높은 대표 표제어로 수렴하세요. 행동보다 앞에 두되, 대상이 있으면 대상 뒤에 둡니다. 예: "반드시 해야 한다"는 사전에서 검색 가능한 의무/필수 계열 표제어로 정규화합니다.
- 강도부사/빈도부사/부정/가능 같은 문법 표지는 핵심 대상-행동 관계를 찾기 전에 먼저 분리해서 해석하세요. 이런 표지가 명사와 동사 사이에 끼어 있어도 통표현 검색 판단을 방해하면 안 됩니다. 예: "비가 많이 내리다"는 먼저 "비가 내리다" 관계를 잡고, 강도 의미는 별도 보조 의미로만 처리합니다.
...
1. 수어사전 표준 표제어 기반 분해 (Exact Dictionary Matching)
   ...
   - 부사형/활용형은 먼저 핵심 대상-행동 관계를 해치지 않는지 판단하세요. "비가 많이 내리다"처럼 강도부사가 통표현 사이에 끼어 있으면 "비가 내리다" 관계를 우선하고, 강도 의미는 필요할 때만 별도 보조 표제어로 남기세요. 일반 동작 수식에서는 사전 검색 가능한 기본 표제어로 환원할 수 있습니다. 예: "많이 먹다" -> ["많다", "먹다"], "빨리" -> "빠르다", "천천히" -> "느리다", "열심히" -> "열심".
   - 파생어 및 구어체 표현은 사전에 존재할 확률이 높은 원초적 단어로 치환하세요. 예: "노래하다" -> "노래", "좋아하다/조아하다" -> "좋다".
   - 사전에서 약한 한자어/문어체 동사는 문맥을 해치지 않는 범위에서 생활에서 쓰는 기본 동사 표제어로 바꾸세요. 특정 단어를 기계적으로 치환하지 말고, 수어사전에서 더 잘 검색될 대표 유의어가 있을 때만 선택하세요.
...
3. 구어체 미사여구 제거 및 시각적 재배치 (수어 어순)
   ...
   - 의무/강조 표지("반드시", "꼭", "필수", "-해야 한다")는 수어사전에서 검색 가능한 대표 유의어로 정규화하고 핵심 행동 바로 앞에 둡니다. 대상이 있으면 [대상] -> [의무/강조] -> [행동] 순서를 유지하세요.
...
Output:
{
  ...
  "ksl_syntax_order": ["운전", "안전", "벨트", "필수", "착용"],
  "facial_expression_token": "NEUTRAL"
}
...
{
  "status": "success",
  "original_text": "비가 많이 내리다.",
  "ksl_syntax_order": ["비", "많다", "내리다"],
  "facial_expression_token": "NEUTRAL"
}
...
function phraseCandidatesFromTerms(terms) {
  ...
  const lexicalTerms = terms.filter(item =>
    ...
    !phraseTransparentModifiers.has(item?.term) &&
...
function cultureQueryVariants(query) {
  ...
  const fallbacks = lexicalFallbacks.get(normalizeKslSearchTerm(query));
  if (fallbacks) variants.push(...fallbacks);
  return variants.filter((variant, index, list) => variant && list.indexOf(variant) === index);
}
