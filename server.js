...
1. 수어사전 표준 표제어 기반 분해 (Exact Dictionary Matching)
   ...
   - [실제 표제어 강제 매핑] 교통수단 배 -> "배, 선박, 선척"; 먹는 과일 배 -> "(과일)배"; 사람 신체 배 -> "배".
   - [실제 표제어 강제 매핑] 타는 자동차 차 -> "차, 자동차, 차량"; 마시는 음료 차 -> "차, 다, 음료".
   - [실제 표제어 강제 매핑] 하늘에서 내리는 눈 -> "눈"; 사람 신체 눈 -> "(신체)눈" 또는 문맥상 더 정확한 신체 표제어.
   - 그 외 다의어가 발생하는 구체물/행동도 임의 괄호가 아니라 실제 사전 표준에 가까운 앞괄호 카테고리 또는 쉼표 나열 표제어로 유추하세요.
...
2. 오타 자동 교정 및 농인 중심의 문맥 판별
   ...
   - "밤"은 다의어입니다. "어젯밤", "오늘 밤", "밤에"처럼 시간 표현이면 "밤"이고, "밤이 먹고 싶어", "밤을 먹다", "군밤"처럼 먹는 대상이면 "(과일)밤"처럼 음식/열매 계열 앞괄호 표제어로 유추하세요.
...
3. 구어체 미사여구 제거 및 시각적 재배치 (수어 어순)
   ...
   - 부정어("부정", "아니다")와 의문사("왜", "무엇", "어디")는 농인이 결론을 확실히 인지할 수 있도록 최종 토큰 배열의 맨 뒤로 보냅니다.
...
Output:
{
  ...
  "ksl_syntax_order": ["학교", "공부", "쉬다", "싶다", "부정"],
  "facial_expression_token": "NEGATION"
}
...
Output:
{
  ...
  "ksl_syntax_order": ["방금", "배, 선박, 선척", "타다", "배", "아프다"],
  "facial_expression_token": "NEUTRAL"
}
...
Output:
{
  ...
  "ksl_syntax_order": ["집", "학교", "(과일)배", "가다", "먹다", "싶다", "부정"],
  "facial_expression_token": "NEGATION"
}
...
Output:
{
  ...
  "ksl_syntax_order": ["너", "마르다", "QUESTION_WHY", "밥", "먹다", "부정"],
