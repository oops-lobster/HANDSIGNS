...
2. 오타 자동 교정 및 농인 중심의 문맥 판별
   ...
   - "밤"은 다의어입니다. "어젯밤", "오늘 밤", "밤에"처럼 시간 표현이면 "밤(시간)"이고, "밤이 먹고 싶어", "밤을 먹다", "군밤"처럼 먹는 대상이면 "밤(열매)"입니다.
   - "안" 문맥 판별은 매우 중요합니다. "안 하다", "안하고", "안 먹다", "공부 안하고"처럼 용언 앞/뒤에서 부정을 만들면 반드시 부정어 "안"으로 출력하고, 절대 "안에서"나 "안(내부)"로 바꾸지 마세요.
...
3. 구어체 미사여구 제거 및 시각적 재배치 (수어 어순)
   ...
   - 부정어("안", "못", "아니다")와 의문사("왜", "무엇", "어디")는 농인이 결론을 확실히 인지할 수 있도록 기본적으로 문장의 맨 뒤로 보냅니다.
   - 단, 한 문장 안에 부정된 행동 뒤로 다른 긍정 행동이 이어지는 경우에는 부정어를 그 행동 바로 뒤에 둡니다. 예: "공부 안하고 쉬고 싶어" -> ["공부", "안", "쉬다", "싶다"].
...
Output:
{
  ...
  "ksl_syntax_order": ["학교", "공부", "안", "쉬다", "싶다"],
  "facial_expression_token": "NEGATION"
}
